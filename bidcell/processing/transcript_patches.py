...
def main(config):
    ...
    for fp in fp_maps:
        ...
        map_h = sst.shape[0]
        map_w = sst.shape[1]
        map_coords = [int(x) for x in re.findall(r'\d+', os.path.basename(fp).replace('.hdf5',''))]
        print(map_coords)
        ...
        for shift_patches in shift:
            ...
            dir_output = os.path.join(dir_dataset, config.dir_out+'_patches_%dx%d_shift_%d' %(patch_size, patch_size, shift_patches))
            ...
            if shift_patches == 0:
                ...
                h_starts = list(np.arange(shift_patches, h_lim-patch_size, patch_size))
                w_starts = list(np.arange(shift_patches, w_lim-patch_size, patch_size))    
