...
def main(config):
    ...
    criterion_pn = PosNegMarkerLoss(json_opts.training_params.pos_weight, json_opts.training_params.neg_weight, device)

    # Optimiser
    if json_opts.training_params.optimizer == "rmsprop":
        optimizer = torch.optim.RMSprop(model.parameters(), lr=json_opts.training_params.learning_rate, 
                                        weight_decay=1e-8)
    ...
    lf = lambda x: (((1 + math.cos(x * math.pi / json_opts.training_params.total_epochs)) / 2) ** 1.0) * 0.95 + 0.05 #cosine
    ...
    for epoch in range(initial_epoch, json_opts.training_params.total_epochs):
        ...
        for step_epoch, (batch_x313, batch_n, batch_sa, batch_pos, batch_neg, coords_h1, coords_w1, nucl_aug, expr_aug_sum) \
            ...
            if (global_step % json_opts.save_freqs.sample_freq) == 0: 
                ...
                patch_fp = experiment_path + '/' + json_opts.experiment_dirs.samples_dir + \
                           "/epoch_%d_%d_%d_%d.png" %(epoch+1, step_epoch, coords_h1, coords_w1)
